...
double PowerSpec(double k)
{
  ...
  if(FDM_On == 1)
    {
      /* Eqn. (8) in Hu, Barkana & Gruzinov (PRL 2000)  */
      kj = pow(FDM_PartMass_in_m22,0.5)*9.0/HubbleParam;
      x = 1.61*pow(FDM_PartMass_in_m22,1.0/18.0)*(3.085678e24 / UnitLength_in_cm)*k/kj;
      Tf = cos(pow(x,3.0))/(1.0+pow(x,8.0));
      power *= Tf * Tf;
    }
